...
def draw_cv2_pointtime(raw_strokes, size=256, lw=6):
	img = np.zeros((BASE_SIZE, BASE_SIZE), np.uint8)
	for t, stroke in enumerate(raw_strokes):
		for i in range(len(stroke[0]) - 1):
			color = (255 - min(i, 255))
...
def image_generator_xd(size, batchsize, ks, lw=6, time_color=True, preprocess_input=None):
	current_k = load_obj(data_path+"current_k.pik")
	current_k -= 1

	while True:
		current_k += 1
		save_obj(current_k,data_path+"current_k.pik")
		...
		try:
			for df in pd.read_csv(filename, chunksize=batchsize):
				...
				for i, raw_strokes in enumerate(df.drawing.values):
					...
					x[i, :, :, 2] = draw_cv2_pointtime(raw_strokes, size=size, lw=lw)																			 	
